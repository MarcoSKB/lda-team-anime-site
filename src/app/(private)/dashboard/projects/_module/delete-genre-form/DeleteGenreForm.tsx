...
import { deleteGenres } from '@/actions/genres'
...
const DeleteGenreForm: React.FC<Props> = ({ updateData, autoTags }) => {
  ...
  const onSubmit: SubmitHandler<AnimeGenreType> = async (data) => {
    const promise = new Promise(async (resolve, reject) => {
      try {
        const res = await deleteGenres(data.genres)
        if (res.type === 'error') {
          reject(res.message)
        }
        if (res.type === 'ok') {
    ...
    toast.promise(promise, {
      loading: 'Удаление жанров...',
      success: 'Жанры успешно удалены',
  ...
  return (
    <div className='flex w-full flex-col gap-3'>
      <span className='text-md'>
        Удалите жанр, чтобы сократить или обновить классификацию контента.
      </span>
      <Form {...form}>
        <form
...
export default memo(DeleteGenreForm)
